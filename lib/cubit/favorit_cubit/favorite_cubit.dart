...
class FavoriteCubit extends Cubit<FavoriteState> {
  ...
  Future<void> toggleFavorite(Movie movie) async{
    if(state is FavoriteLoaded){
      ...
      currentFavorites.removeWhere((fav) => fav.id == movie.id); //remove movie from list also
      emit(FavoriteLoaded(currentFavorites)); // update the loaded state 
    }
    else{
     
      await favoriteRepository.addFavMovie(movie); // Add movie to favorites in firestore
